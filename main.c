...
int main(void) {
    ...
    while(1) {
        ...
        case START:
            ...
            drawCenteredString(GBA_WIDTH/2, GBA_HEIGHT/3, 0, 0,
             "Press Start (Enter) to start/pause", GREEN);

            drawCenteredString(GBA_WIDTH/2, 2*GBA_HEIGHT/3, 0, 0, "Select (BACKSPACE) to reset.", GREEN);
        ...
        case APP_EXIT_NODRAW:
            ...
            if(KEY_DOWN(BUTTON_START, currentButtons) || KEY_DOWN(BUTTON_SELECT, currentButtons)){
                state = START;
            }
        ...
        case PAUSED:
            waitForVBlank();
            drawCenteredString(GBA_WIDTH/2, GBA_HEIGHT/4, 0, 0, "GAME PAUSED", GREEN);
            if(KEY_JUST_PRESSED(BUTTON_SELECT, currentButtons, previousButtons)){
                state = START;
            }else if(KEY_JUST_PRESSED(BUTTON_START, currentButtons, previousButtons)){
                fullDrawAppState(&currentAppState);
